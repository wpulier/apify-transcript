...
def validate_quality(audio_path: Path, canonical: dict[str, Any], provider: str, model: str, mode: str, retry_history: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if mode == "authoritative" and speech_minutes >= 1 and word_count < MIN_SHORT_AUDIO_WORDS:
        failures.append(f"very low word count: {word_count} words across {speech_minutes:.1f} detected minute(s)")
...
def recovery_prompt(config: TranscriptConfig) -> str:
    terms = ", ".join(config.keyterms[:40])
    glossary = f" Preserve these domain terms when present: {terms}." if terms else ""
    return (
        "Transcribe every intelligible spoken or sung word in order. "
        "Do not summarize, compress, paraphrase, or omit repeated phrases. "
        "If the audio is music or a song, transcribe the audible lyrics as text. "
        "Keep natural line breaks when they help readability."
        f"{glossary}"
    )


def add_single_segment_if_needed(canonical: dict[str, Any], duration: float | None) -> dict[str, Any]:
    text = clean_text(canonical.get("text"))
    if text and not canonical.get("segments"):
        canonical["segments"] = [
            {
                "id": "prompted_recovery_0",
                "start": 0.0,
    ...
    if duration is not None:
        canonical["duration"] = float(duration)
    canonical["recovery_mode"] = "prompted_text"
    return canonical


def create_openai_prompted_transcript(client: OpenAI, audio_path: Path, config: TranscriptConfig, model: str) -> dict[str, Any]:
    ...
    with audio_path.open("rb") as audio_file:
        response = client.audio.transcriptions.create(
            ...
            language=config.language or None,
            prompt=recovery_prompt(config),
        )
    canonical = normalize_openai_response(response, "openai", model)
    return add_single_segment_if_needed(canonical, duration)
...
def transcribe_openai_recovery_chunk(api_key: str, chunk_path: Path, config: TranscriptConfig, model: str, index: int, offset: float) -> tuple[int, dict[str, Any]]:
    client = OpenAI(api_key=api_key, timeout=TRANSCRIPT_REQUEST_TIMEOUT_SECONDS)
    canonical = create_openai_prompted_transcript(client, chunk_path, config, model)
    return index, offset_canonical(canonical, offset, index)
...
def transcribe_openai_prompted_recovery(media_path: Path, config: TranscriptConfig, temp_dir: Path, log: LogFn) -> tuple[dict[str, Any], str]:
    if not config.openai_api_key:
        raise RuntimeError("OpenAI transcript output requires an OpenAI API key")
    model = config.openai_recovery_model
    client = OpenAI(api_key=config.openai_api_key, timeout=TRANSCRIPT_REQUEST_TIMEOUT_SECONDS)
    upload_path, cleanup_path = prepare_openai_audio(media_path, temp_dir)
    chunk_dir = None
    try:
        if upload_path.stat().st_size <= OPENAI_UPLOAD_TARGET_BYTES:
            log(f"OpenAI prompted recovery upload: {upload_path.name} ({upload_path.stat().st_size / (1024 * 1024):.1f} MB)")
            return create_openai_prompted_transcript(client, upload_path, config, model), model

        chunks, chunk_dir = prepare_chunks(upload_path, temp_dir, OPENAI_AUTHORITATIVE_CHUNK_SECONDS)
        log(f"OpenAI prompted recovery using {len(chunks)} chunks with concurrency {config.concurrency}")
        offsets = []
        offset = 0.0
        for chunk in chunks:
            offsets.append(offset)
        ...
        with ThreadPoolExecutor(max_workers=config.concurrency) as executor:
            futures = {
                executor.submit(transcribe_openai_recovery_chunk, config.openai_api_key, chunk, config, model, index, offsets[index]): index
                for index, chunk in enumerate(chunks)
            }
            for future in as_completed(futures):
                index, canonical = future.result()
                results[index] = canonical
                log(f"OpenAI recovery chunk {index + 1}/{len(chunks)} complete")
        canonical = merge_canonicals([results[index] for index in range(len(chunks))], "openai", model)
        canonical["recovery_mode"] = "prompted_text"
...
def transcribe_with_provider(media_path: Path, config: TranscriptConfig, provider: str, temp_dir: Path, log: LogFn, retry_history: list[dict[str, Any]]) -> TranscriptBundle:
    if provider == "openai":
        if config.quality_mode == "fast":
            ...
            if quality.get("quality_status") == "failed_qa":
                ...
                log("OpenAI diarized transcript failed QA; retrying with prompted recovery")
                canonical, model = transcribe_openai_prompted_recovery(media_path, config, temp_dir, log)
